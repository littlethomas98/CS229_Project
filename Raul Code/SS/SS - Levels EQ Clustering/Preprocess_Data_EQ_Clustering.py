...
def NormalizeData(LabeledData):
    ...
    # SO2 Concentrations
    min_SO2 = LabeledData['Daily Max 1-hour SO2 Concentration'].min()
    max_SO2 = LabeledData['Daily Max 1-hour SO2 Concentration'].max()
    LabeledData['Daily Max 1-hour SO2 Concentration'] = (LabeledData['Daily Max 1-hour SO2 Concentration'] - min_SO2) / (max_SO2 - min_SO2)
...
def split_data(ClusteredData, train_ratio, test_ratio):
    ...
    # Drop Cluster_ID of Labels
    Trainning_Data = Trainning_Data.drop(columns = ['Cluster_ID'])
    Validation_Data = Validation_Data.drop(columns = ['Cluster_ID'])
    Testing_Data = Testing_Data.drop(columns = ['Cluster_ID'])
...
def main():
    ...
    LabeledData.to_csv('All_Data_Normalized.csv', index = False)
